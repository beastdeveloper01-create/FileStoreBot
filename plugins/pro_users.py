...
@Client.on_message(filters.command(['addcredit', 'delcredit', 'setcredit', 'resetcredit']) & filters.private)
async def credit_management_command(client: Client, message: Message):
    ...
    try:
        if command == 'resetcredit':
            if len(message.command) != 2:
                return await message.reply_text("<b>Usage:</b> /resetcredit <user_id>")
            user_id = int(message.command[1])
            await client.mongodb.set_credits(user_id, 0)
            await message.reply_text(f"✅ User `{user_id}` credits have been reset to 0.")
            try:
                await client.send_message(
                    user_id,
                    "🔄 **Credits Reset!**\n\nAn admin has reset your credit balance to **0**."
                )
            except Exception as e:
                await message.reply_text(f"⚠️ Could not notify the user: `{e}`")
            return
        
        if len(message.command) != 3:
            return await message.reply_text(f"<b>Usage:</b> /{command} <user_id> <amount>")
            
        user_id = int(message.command[1])
        amount = int(message.command[2])

        if command == 'addcredit':
            await client.mongodb.update_credits(user_id, amount)
            new_balance = await client.mongodb.get_credits(user_id)
            await message.reply_text(f"✅ Added {amount} credits to user `{user_id}`. New balance: {new_balance}.")
            try:
                await client.send_message(
                    user_id,
                    f"㊣ **Cʀᴇᴅɪᴛs Aᴅᴅᴇᴅ!**\n\nAn admin has added **{amount}** credits to your account.\n\nYour new balance is: **{new_balance}** credits."
                )
            except Exception as e:
                await message.reply_text(f"⚠️ Could not notify the user: `{e}`")

        elif command == 'delcredit':
            await client.mongodb.update_credits(user_id, -amount)
            new_balance = await client.mongodb.get_credits(user_id)
            await message.reply_text(f"✅ Removed {amount} credits from user `{user_id}`. New balance: {new_balance}.")
            try:
                await client.send_message(
                    user_id,
                    f"㋡ **Cʀᴇᴅɪᴛs Rᴇᴍᴏᴠᴇᴅ!**\n\nAn admin has removed **{amount}** credits from your account.\n\nYour new balance is: **{new_balance}** credits."
                )
            except Exception as e:
                await message.reply_text(f"⚠️ Could not notify the user: `{e}`")

        elif command == 'setcredit':
            await client.mongodb.set_credits(user_id, amount)
            await message.reply_text(f"✅ User `{user_id}` credits have been set to {amount}.")
            try:
                await client.send_message(
                    user_id,
                    f"📊 **Credits Updated!**\n\nAn admin has set your credit balance to **{amount}** credit(s)."
                )
            except Exception as e:
                await message.reply_text(f"⚠️ Could not notify the user: `{e}`")

    except (ValueError, IndexError):
        await message.reply_text("Invalid format. Please use numbers for user ID and amount.")
    except Exception as e:
